...
class ApacheController extends AbstractController {
		...
		try {
			...
			foreach ( $lines as $line ) {
				...
				if ( count( $ex ) === 4 ) {
					...
					$arr[] = 'RewriteRule ^' . $fromPath . '$ ' . $toUrl . $qsFlag . ' [R=' . $status . ',L]';
					$arr[] = '';
				} elseif ( count( $ex ) === 3 ) {
	...
	public function apiApacheBasicAuth( Request $request ): JsonResponse {
		try {
			...
			$allowUrls      = isset( $body['allowUrls'] ) && $body['allowUrls'] === '1';
			$allowUrlList   = [];
			if ( $allowUrls && ! empty( $body['allowUrlList'] ) ) {
				$allowUrlList = array_values( array_filter(
	...
	private function processFromPath( array $from ): string {
		...
		// Escape special regex characters except $ (rule builder adds end anchor separately)
